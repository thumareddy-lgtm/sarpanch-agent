...
# ── Config ───────────────────────────────────────────────────
VILLAGE_NAME  = os.environ.get("VILLAGE_NAME",  "Ramapuram Village")
SARPANCH_NAME = os.environ.get("SARPANCH_NAME", "Sri Ravi Kumar")
MANDAL        = os.environ.get("MANDAL",        "Nalgonda Mandal")
...
.stats{display:flex;gap:8px;padding:14px 12px 0;flex-wrap:wrap}
...
@media(max-width:768px){
  ...
  .complaint-card{background:#fff;border-radius:10px;padding:14px;margin:8px 12px;box-shadow:0 1px 4px rgba(0,0,0,.08);border-left:4px solid var(--amber)}
  ...
  .card-id{font-size:11px;color:var(--sub);font-weight:600}
  .card-name{font-size:15px;font-weight:700;color:var(--text)}
  .card-detail{font-size:12px;color:var(--sub)}
