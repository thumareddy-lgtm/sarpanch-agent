...
@media(max-width:768px){
  ...
  .sec{margin:10px}
  .sh{padding:10px 12px;font-size:13px;flex-direction:column;align-items:flex-start;gap:3px}
  .tb{padding:10px 12px;height:auto;flex-direction:column;align-items:flex-start;gap:4px}
  .dsk{display:none!important}
  .mc{background:#fff;border-radius:10px;padding:14px;margin:10px;box-shadow:0 1px 4px rgba(0,0,0,.08);border-left:4px solid var(--amber)}
  .mc.high{border-left-color:var(--red)}
  .mc.medium{border-left-color:var(--amber)}
  .mc.low{border-left-color:var(--green)}
  .mc-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:5px}
  .mc-id{font-size:11px;color:var(--sub);font-weight:600}
  .mc-name{font-size:15px;font-weight:700}
  .mc-info{font-size:12px;color:var(--sub);margin-top:3px}
  .mc-acts{display:flex;gap:6px;margin-top:10px;flex-wrap:wrap}
...
<div class="sec">
  ...
  {% if ac %}
  <div class="dsk"><table><thead><tr><th>#</th><th>ID</th><th>Name</th><th>Category</th><th>Location</th><th>Priority</th><th>Filed</th><th>Status</th><th>Actions</th></tr></thead><tbody>
  {% for x in ac %}<tr>
  ...
  <div class="mob">{% for x in ac %}
  <div class="mc {{ x.priority }}">
    <div class="mc-row"><span class="mc-id">{{ x.id }}</span><span class="badge {{ x.status }}">{{ x.status.replace('_',' ').title() }}</span></div>
    <div class="mc-name">{{ x.name }}</div>
    <div class="mc-info">📋 {{ x.category }} · 📍 {{ x.location }}</div>
    <div class="mc-info">⚡ {{ x.priority|upper }} · 📅 {{ x.filed_at }}</div>
    <div class="mc-info">📞 {{ x.phone }}</div>
    <div class="mc-acts">
      {% if x.status=='pending' %}<a href="/caction/{{ x.id }}/in_review" class="btn bb">🔍 Review</a>{% endif %}
      {% if x.status=='in_review' %}<a href="/caction/{{ x.id }}/in_progress" class="btn ba">▶ Start</a>{% endif %}
      {% if x.status=='in_progress' %}<a href="/caction/{{ x.id }}/resolved" class="btn bg">✓ Done</a>{% endif %}
...
<div class="sec">
  ...
  {% if ac %}<div class="dsk"><table><thead><tr><th>#</th><th>ID</th><th>Name</th><th>Type</th><th>Purpose</th><th>Filed</th><th>Status</th><th>Actions</th></tr></thead><tbody>
  {% for x in ac %}<tr>
    <td>{{ loop.index }}</td><td><strong>{{ x.id }}</strong></td>
    <td>{{ x.name }}<br><small style="color:#888">{{ x.phone }}</small></td>
  ...
  <div class="mob">{% for x in ac %}
  <div class="mc medium">
    <div class="mc-row"><span class="mc-id">{{ x.id }}</span><span class="badge {{ x.status }}">{{ x.status.title() }}</span></div>
    <div class="mc-name">{{ x.name }}</div>
    <div class="mc-info">📄 {{ x.type }} · 🎯 {{ x.purpose }}</div>
    <div class="mc-info">📅 {{ x.filed_at }} · 📞 {{ x.phone }}</div>
    <div class="mc-acts">
      {% if x.status=='pending' %}<a href="/certaction/{{ x.id }}/processing" class="btn bb">🔄 Process</a>{% endif %}
      {% if x.status=='processing' %}<a href="/certaction/{{ x.id }}/ready" class="btn bg">✓ Ready</a>{% endif %}
